...
layui.use(['form','layer','table','laytpl','layuiRequest'],function(){
    var form = layui.form,
    ...
    function editDict(data) {
    	var editDictIndex = layui.layer.open({
            type: 2,
            title: '编辑字典', 		// 不显示标题栏
            closeBtn: 1,			// 关闭按钮
            shade: 0, 				// 遮罩
            ...
            scrollbar: false, 		// 是否允许浏览器出现滚动条
            maxmin: true, 			// 最大最小化
            id: 'LAY_EditDict', 	// 用于控制弹层唯一标识
            moveType: 1,
            content: [ctx + '/sys/dict/edit/' + data.id],
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                setTimeout(function(){
                    layui.layer.tips('点击此处返回字典列表', '.layui-layer-setwin .layui-layer-close', {
    	...
    	layui.layer.full(editDictIndex);
        window.sessionStorage.setItem("editDictIndex", editDictIndex);
        $(window).on("resize",function() {
        	layui.layer.full(window.sessionStorage.getItem("editDictIndex"));
        })
    ...
    function delDict(data) {
        layer.msg('确定要删除此字典?', {
            ...
            btn1: function(index, layero) {
            	layuiRequest.delDict(data, index, ctx + '/sys/dict/del', dictListtIns);
            },
